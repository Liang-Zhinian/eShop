using eShop.Core.Models.Location;
using eShop.Core.Services.Location;
using eShop.Windows.Services;
using System;
...
namespace eShop.Windows.Services
{
    public class LocationServiceImplementation : ILocationServiceImplementation
    {
        ...
        {
            var timeoutMilliseconds = timeout.HasValue ? (int)timeout.Value.TotalMilliseconds : eShop.Windows.Helpers.Timeout.Infinite;
            if (timeoutMilliseconds < 0 && timeoutMilliseconds != eShop.Windows.Helpers.Timeout.Infinite)
            ...
            var timer = new eShop.Windows.Helpers.Timeout(timeoutMilliseconds, pos.Cancel);
