...
export default class DateTimePickerIOS extends React.PureComponent {
  static propTypes = {
    ...
    onDismissAfterConfirm: PropTypes.func,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.props.minuteInterval !== nextProps.minuteInterval) {
      this.setState({
  ...
  _handleOnModalHide = () => {
    if (this.confirmed) {
      this.props.onDismissAfterConfirm(this.state.date);
    }
  };
  ...
  render() {
    ...
    return (
      <ReactNativeModal
        ...
        onModalShow={() => {
          setTimeout(() => {
            this.setState({ minuteInterval: 1 }, () => {
              // this.forceUpdate();
            });
          }, 10);
          setTimeout(() => {
