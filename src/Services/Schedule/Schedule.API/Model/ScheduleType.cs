...
using System.Linq;
using Schedule.API.Infrastructure.Exceptions;

namespace SaaSEqt.eShop.Services.Schedule.API.Model
{
    public class ScheduleType : Enumeration
    {
        ...
        public static ScheduleType Appointment = new ScheduleType(4, nameof(Appointment).ToLowerInvariant());
        public static ScheduleType Resource = new ScheduleType(5, nameof(Resource).ToLowerInvariant());
        ...
        new[] { All, DropIn, Enrollment, Appointment, Resource, Media, Arrival };
        ...
        public static ScheduleType FromName(string name)
        {
            ...
            if (state == null)
            {
                throw new ScheduleDomainException($"Possible values for ScheduleType: {String.Join(",", List().Select(s => s.Name))}");
            }
