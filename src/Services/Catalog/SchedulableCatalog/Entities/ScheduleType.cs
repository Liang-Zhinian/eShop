...
using System.Linq;
using SaaSEqt.eShop.Services.SchedulableCatalog.Infrastructure.Exceptions;
using SaaSEqt.eShop.Services.SchedulableCatalog.SeedWork;
...
namespace SaaSEqt.eShop.Services.SchedulableCatalog.Entities
{
    ...
    public class ScheduleType : Enumeration
    {
        ...
        public static ScheduleType FromName(string name)
        {
            ...
            if (state == null)
            {
                throw new SchedulableCatalogDomainException($"Possible values for ScheduleType: {String.Join(",", List().Select(s => s.Name))}");
            }
